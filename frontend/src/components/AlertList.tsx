...
export default function AlertList({ triggers }: AlertListProps) {
    const getSeverityVariant = (severity: DetectedTrigger['severity']): 'secondary' | 'default' | 'destructive' => {
        switch (severity) {
        ...
        <Card role="region" aria-label="Detected threats" className="border-primary/30 bg-card/50">
            ...
            <CardContent>
                <Accordion type="multiple" className="w-full">
                    {(Object.entries(groupedTriggers) as [TriggerType, DetectedTrigger[]][]).map(([type, typeTriggers], typeIndex) => {
